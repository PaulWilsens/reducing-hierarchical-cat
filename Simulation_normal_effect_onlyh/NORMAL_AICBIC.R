...
source("Functions/NORMAL_AICBIC_functions.R")

#### Load previous result

result = readRDS("NORMAL_result")
...
BIC_frame_NORMAL = data.frame()
AIC_frame_NORMAL = data.frame()

structure_completecollapse = readRDS("true_noeffect")

for (i in 1:length(result)) {
  
  data = read.csv(paste0("Data/Data_3fact_SIMULATED_",i,".csv"),header = T)
  ...
  for (j in 1:5) {
    ...
    if(identical(temp_final,structure_completecollapse)) {
      ...
      BIC_frame_NORMAL = rbind(BIC_frame_NORMAL,temp)
      AIC_frame_NORMAL = rbind(AIC_frame_NORMAL,temp2)
