...
source("Functions/POISSON_AICBIC_functions.R")

#### Load previous result

result = readRDS("POISSON_result")
...
BIC_frame_POISSON = data.frame()
AIC_frame_POISSON = data.frame()

structure_completecollapse = readRDS("true_noeffect")

for (i in 1:length(result)) {
  
  data = read.csv(paste0("Data/Data_3fact_POISSON_SIMULATED_",i,".csv"),header = T)
  ...
  for (j in 1:5) {
    ...
    if(identical(temp_final,structure_completecollapse)) {
      ...
      BIC_frame_POISSON = rbind(BIC_frame_POISSON,temp)
      AIC_frame_POISSON = rbind(AIC_frame_POISSON,temp2)
