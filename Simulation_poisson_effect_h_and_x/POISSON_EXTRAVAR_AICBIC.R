...
source("Functions/POISSON_EXTRAVAR_AICBIC_functions.R")

#### Load previous result

result = readRDS("POISSON_EXTRAVAR_result")
...
BIC_frame_POISSON_EXTRAVAR = data.frame()
AIC_frame_POISSON_EXTRAVAR = data.frame()

structure_completecollapse = readRDS("true_noeffect")

for (i in 1:length(result)) {
  
  data = read.csv(paste0("Data/Data_3fact_POISSON_EXTRAVAR_SIMULATED_",i,".csv"),header = T)
  ...
  for (j in 1:5) {
    ...
    if(identical(temp_final,structure_completecollapse)) {
      ...
      model_new = glm(data$y ~ data$x2_simul + data$x3_simul + data$x4_simul,family = poisson(link = 'log'))
      ...
      BIC_frame_POISSON_EXTRAVAR = rbind(BIC_frame_POISSON_EXTRAVAR,temp)
      AIC_frame_POISSON_EXTRAVAR = rbind(AIC_frame_POISSON_EXTRAVAR,temp2)
