...
source("Functions/POISSON_EXTRAVAR_SIMUL_functions.R")
...
for (l in 1:100) {
  ...
  data = read.csv(paste0("Data/Data_3fact_POISSON_EXTRAVAR_SIMULATED_",l,".csv"),header = T)
  ...
  for (k in 1:5) {
    ...
    model %>% 
      compile(loss = "poisson", optimizer = "adam")
    ...
    for (j in 1:tree_levels) {
      ...
      assign(paste0("clusters_lev",j),horizontal_clustering(j))
      listH[[length(listH)+1]] = get(paste0("clusters_lev",j))
      
      if (j < length(embedding_list)) {
        assign(paste0("fake_lev",j+1),vertical_clustering(j))
