...
source("Functions/NORMAL_EXTRAVAR_SIMUL_functions.R")
...
for (l in 1:100) {
  ...
  data = read.csv(paste0("Data/Data_3fact_EXTRAVAR_SIMULATED_",l,".csv"),header = T)
  data$leaf     = as.factor(data$leaf)
  data$leaf_int = as.numeric(data$leaf_int)
  data$y = scale(data$y)
  data$x2_simul = scale(data$x2_simul)
  data$x3_simul = scale(data$x3_simul)
  data$x4_simul = scale(data$x4_simul)
  ...
  for (k in 1:5) {
    ...
    x2_inp = layer_input(shape=c(1),name='x2_inp')
    x3_inp = layer_input(shape=c(1),name='x3_inp')
    x4_inp = layer_input(shape=c(1),name='x4_inp')
    ...
    model1 = layer_concatenate(c(x2_inp,x3_inp,x4_inp, Leaf_out)) %>%    
      layer_dense(units=2,kernel_regularizer = regularizer_l2(0.01)) %>%             
      layer_dense(units=1)
    
    model = keras::keras_model(inputs = c(x2_inp,x3_inp,x4_inp, Leaf), outputs = model1)
    
    
    model %>% 
      compile(loss = "mse", optimizer = "adam")
    
    input = list(as.matrix(data$x2_simul),as.matrix(data$x3_simul),as.matrix(data$x4_simul),as.matrix(data$leaf_int))
    
    
    fit = model %>% 
      fit(x = input, y= as.matrix(data$y), epochs = 50, batch_size = 25)
